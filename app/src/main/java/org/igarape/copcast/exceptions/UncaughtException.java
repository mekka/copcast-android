...
/**
 * Created by brunosiqueira on 26/07/16.
 */
public class UncaughtException implements Thread.UncaughtExceptionHandler {
    ...
    public void sendErrorMail(final StringBuilder errorContent)
    {
        ...
        new Thread()
        {
            ...
            {
                Looper.prepare();
                builder.setTitle(context.getString(R.string.title_crash));
                builder.create();
                ...
                builder.setPositiveButton(context.getString(R.string.report_crash), new DialogInterface.OnClickListener()
                {
                    @Override
                    public void onClick(DialogInterface dialog, int which)
                    {
                        Intent sendIntent = new Intent(Intent.ACTION_SEND);
                        String subject = context.getString(R.string.crash_subject);
                        StringBuilder body = new StringBuilder("Information");
                        body.append('\n').append('\n');
                        body.append(errorContent).append('\n').append('\n');
                        // sendIntent.setType("text/plain");
                        sendIntent.setType("message/rfc822");
                        sendIntent.putExtra(Intent.EXTRA_EMAIL, new String[] {BuildConfig.EMAIL});
                ...
                builder.setMessage(context.getString(R.string.message_crash));
                builder.show();
                Looper.loop();
            }
        }.start();
